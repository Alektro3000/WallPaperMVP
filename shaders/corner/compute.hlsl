...
void main(uint3 dtid : SV_DispatchThreadID)
{
    ...
    if (p.Age < 0)
    {
        ...
        if (spawnIndex < Emitter[0].SpawnCountThisFrame)
        {
            ...
            float2 rnd = normalize(float2(Random(seed), Random(3*seed+1))-0.5f);
    ...
    {
        p.Velocity += p.Position * DeltaTime * 0.1f;
    ...
    else
    {
        ...
        p.Color = float4(Color, scaledAge > 0.8f ? 5-5*scaledAge : scaledAge*1.25);
    }
